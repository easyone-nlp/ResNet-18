## https://github.com/ymerkli/fgsm-attack/tree/master 
...
class FGSMAttack:
    ...
    def perturb(self, x, eps, grad):
        ...
        clamp_min, clamp_max = self._prepare_clamp(x)
        if clamp_min is not None and clamp_max is not None:
            x_prime = torch.max(torch.min(x_prime, clamp_max), clamp_min)
        return x_prime
    ...
    def run(self):
        ...
        for eps_real in self.epsilons:
            ...
            for data, label in self.test_dataloader:
                ...
                if self.is_targeted:
                    target_labels = torch.full_like(label[valid_mask], fill_value=self.target, device=self.device)
                    loss = criterion(output[valid_mask], target_labels)
                else:
                    loss = criterion(output[valid_mask], label[valid_mask])
                ...
                perturbed_data[valid_mask] = self.perturb(data[valid_mask].detach(), eps_real, data_grad[valid_mask])
